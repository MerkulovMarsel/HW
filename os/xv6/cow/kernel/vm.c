...
int cow_trap(pagetable_t pagetable, uint64 va) {
  pte_t *pte;
  uint64 pa;
  uint flags;
  char *mem;
  ...
  pte = walk(pagetable, va, 0);
  if (pte == 0 || (*pte & PTE_V) == 0 || (*pte & PTE_BLK) == 0) return -1;

  pa = PTE2PA(*pte);
  flags = (PTE_FLAGS(*pte) & ~PTE_BLK) | PTE_W;

  int ref_count = get_ref_count(pa);
  if (ref_count <= 0) return -1;

  if (ref_count == 1) {
    *pte = (*pte & ~PTE_BLK) | PTE_W;
    return 0;
  }
...
int copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len) {
  ...
  while (len > 0) {
    ...
    if ((*pte & PTE_W) == 0 && (*pte & PTE_BLK) != 0) {
      if (cow_trap(pagetable, va0) != 0) return -1;
