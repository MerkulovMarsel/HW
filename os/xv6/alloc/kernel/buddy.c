...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  alloc:");
    bd_print_vector(bd_sizes[k].xor, NBLK(k) / 2);
    if (k > 0) {
      printf("  split:");
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_invert(bd_sizes[k].xor, blk_index(k, p) / 2);
  ...
  for (; k > fk; k--) {
    ...
    bit_invert(bd_sizes[k - 1].xor, blk_index(k - 1, q) / 2);
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_invert(bd_sizes[k].xor, bi / 2);
    if (bit_isset(bd_sizes[k].xor, bi / 2)) {
      break;
    }
...
int bd_initfree_pair(int k, int bi, uint8 is_right) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_isset(bd_sizes[k].xor, bi / 2)) {
